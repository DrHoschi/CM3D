...
let authoritativeArc = authoritativeSketch.data.arcs[created.arcId];
assert.ok(authoritativeArc);
assert.equal(authoritativeArc.startPointId,created.startPointId);
assert.equal(authoritativeArc.endPointId,created.endPointId);
assert.deepEqual(authoritativeArc.control,{x:1,y:1});
assert.deepEqual(authoritativeSketch.data.points[created.startPointId],{pointId:created.startPointId,x:0,y:0});
assert.deepEqual(authoritativeSketch.data.points[created.endPointId],{pointId:created.endPointId,x:2,y:0});
...
const startId = authoritativeArc.startPointId, endId = authoritativeArc.endPointId;
assert.equal(store.setSketchArcGeometry(sketch.objectId,authoritativeArc.arcId,{start:{x:-1,y:0},end:{x:3,y:0},control:{x:1,y:2}}),true);
assert.equal(store.undoStack.length,2);
authoritativeSketch = store.getObject(sketch.objectId);
authoritativeArc = authoritativeSketch.data.arcs[created.arcId];
assert.equal(authoritativeArc.startPointId,startId);
assert.equal(authoritativeArc.endPointId,endId);
assert.deepEqual(authoritativeSketch.data.points[startId],{pointId:startId,x:-1,y:0});
assert.deepEqual(authoritativeSketch.data.points[endId],{pointId:endId,x:3,y:0});
assert.deepEqual(authoritativeArc.control,{x:1,y:2});
...
authoritativeSketch = store.getObject(sketch.objectId);
assert.equal(authoritativeSketch.data.arcs[second.arcId].startPointId,startId);
assert.deepEqual(authoritativeSketch.data.arcs[second.arcId].control,{x:6,y:1},'connect must not change arc control');
...
authoritativeSketch = store.getObject(sketch.objectId);
assert.deepEqual(authoritativeSketch.data.arcs[second.arcId].control,{x:6,y:1},'disconnect must not change arc control');
...
assert.match(main,/const BUILD_ID = '(?:WD-21C\.[7-9](?:-R\d+)?|WD-21D\.[1-9](?:-R\d+)?|WD-21G\.1)'/);
