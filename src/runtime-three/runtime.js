...
export class ThreeRuntime {
  ...
  clearModelRoot(){
    ...
    this.modelRoot.traverse(node=>{if(node===this.modelRoot)return;node.geometry?.dispose?.();if(Array.isArray(node.material))node.material.forEach(m=>m?.dispose?.());else node.material?.dispose?.();});
  ...
  rebuild(){
    ...
    for(const object of Object.values(objects)){
      const node=this.objectMap.get(object.objectId),parent=object.parentId?this.objectMap.get(object.parentId):null;
      (parent??this.modelRoot).add(node);
    }
    this.syncSelection();
  }
