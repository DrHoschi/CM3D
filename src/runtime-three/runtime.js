...
export class ThreeRuntime {
  ...
  focusSelection(){const selectedElement=this.store.selection.sketchElement;const id=selectedElement?.sketchId??this.store.selection.activeObjectId;const node=id?this.objectMap.get(id):null;if(!node)return;node.updateWorldMatrix(true,true);let focusNode=node;if(selectedElement){let matched=null;node.traverse(child=>{if(matched)return;const meta=child.userData?.cm3dSketchElement;if(meta?.sketchId===selectedElement.sketchId&&meta?.kind===selectedElement.kind&&meta?.elementId===selectedElement.elementId)matched=child;});if(matched)focusNode=matched;}const box=new THREE.Box3().setFromObject(focusNode);if(box.isEmpty())box.setFromCenterAndSize(focusNode.getWorldPosition(new THREE.Vector3()),new THREE.Vector3(1,1,1));const center=box.getCenter(new THREE.Vector3()),size=box.getSize(new THREE.Vector3()),maxSize=Math.max(size.x,size.y,size.z,0.0000001);const fov=THREE.MathUtils.degToRad(this.camera.fov),distance=Math.max(maxSize/(2*Math.tan(fov/2))*1.6,0.000001);const direction=this.camera.position.clone().sub(this.orbit.target);if(direction.lengthSq()<1e-20)direction.set(1,0.75,1);direction.normalize();this.orbit.target.copy(center);this.camera.position.copy(center).addScaledVector(direction,distance);this.updateCameraRange();this.updateGrid();this.orbit.update();this.store.emit('focusCompleted',{objectId:id});}
