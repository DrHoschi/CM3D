...
store.selectRef=(ref,notify=true,additive=false)=>{if(!ref)return false;if(ref.targetKind==='SKETCH_ELEMENT')return selectRegisteredSketchTarget(ref.ownerId,ref.subTargetId,ref.targetId,notify);if(ref.targetKind==='SKETCH_POINT')return selectRegisteredSketchTarget(ref.ownerId,'point',ref.targetId,notify);if(['PROFILE','PATH'].includes(ref.targetKind)){const resolution=resolveStableReference(store,ref);if(resolution.state!==ReferenceState.RESOLVED)return false;const preserved=additive?[...(store.selection.sketchElements??[]).filter(item=>item.sketchId===ref.ownerId)]:[];if(additive&&store.setSketchMultiSelectEnabled)store.setSketchMultiSelectEnabled(true,false);legacyObjectSelect(ref.ownerId,false,false);const next={sketchId:ref.ownerId,kind:ref.targetKind.toLowerCase(),elementId:ref.targetId};if(additive){store.selection.sketchElements=preserved;if(!store.selection.sketchElements.some(item=>item.kind===next.kind&&item.elementId===next.elementId))store.selection.sketchElements.push(next);}else store.selection.sketchElements=[next];store.selection.sketchElement=next;if(notify)store.emit('selectionChanged',{sketchElement:structuredClone(next)});syncSelectionRefs();return true;}if(!['OBJECT','SKETCH'].includes(ref.targetKind))return false;const object=store.getObject(ref.targetId);if(!object)return false;if(ref.targetKind==='SKETCH'&&object.type!=='sketch')return false;if(ref.targetKind==='OBJECT'&&object.type==='sketch')return false;legacyObjectSelect(ref.targetId,notify,additive);syncSelectionRefs();return true;};
