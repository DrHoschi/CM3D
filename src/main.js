...
import { resolveStableReference, ReferenceState } from './application/stable-reference.js';
import { installProfilePathSelectionProjection } from './ui/profile-path-selection.js';
import { installProfilePathIdentityRegistrationLifecycle } from './application/profile-path-identity-registration.js';

const BUILD_ID = 'WD-21G.1';
...
const sketchMutationContract=installSketchMutationContract(store); const genericSketchElementMutationContract=installGenericSketchElementMutationContract(store); const sketchArcCreationContract=installSketchArcCreationContract(store); const sketchSplineCreationContract=installSketchSplineCreationContract(store); const genericEndpointConnectivityContract=installGenericEndpointConnectivityContract(store); const sketchCircleIntegration=installSketchCircleIntegration(store,runtime,appUI); const sketchArcIntegration=installSketchArcIntegration(store,runtime,appUI); const sketchSplineIntegration=installSketchSplineIntegration(store,runtime,appUI); const sketchMultiSelection=installSketchMultiSelection(store,runtime,appUI);
const syncSelectionRefs=()=>{const sketchElements=Array.isArray(store.selection.sketchElements)&&store.selection.sketchElements.length?store.selection.sketchElements:(store.selection.sketchElement?[store.selection.sketchElement]:[]);const refs=sketchElements.length?sketchElements.map(item=>({targetKind:item.kind==='point'?'SKETCH_POINT':item.kind==='profile'?'PROFILE':item.kind==='path'?'PATH':'SKETCH_ELEMENT',ownerId:item.sketchId,targetId:item.elementId,...(['point','profile','path'].includes(item.kind)?{}:{subTargetId:item.kind})})):store.selection.selectedObjectIds.map(objectId=>({targetKind:store.getObject(objectId)?.type==='sketch'?'SKETCH':'OBJECT',ownerId:objectId,targetId:objectId}));store.selection.refs=refs;store.selection.primaryRef=refs.at(-1)??null;};
...
store.selectRef=(ref,notify=true,additive=false)=>{if(!ref)return false;if(ref.targetKind==='SKETCH_ELEMENT')return selectRegisteredSketchTarget(ref.ownerId,ref.subTargetId,ref.targetId,notify);if(ref.targetKind==='SKETCH_POINT')return selectRegisteredSketchTarget(ref.ownerId,'point',ref.targetId,notify);if(['PROFILE','PATH'].includes(ref.targetKind)){const resolution=resolveStableReference(store,ref);if(resolution.state!==ReferenceState.RESOLVED)return false;const preserved=additive?[...(store.selection.sketchElements??[]).filter(item=>item.sketchId===ref.ownerId)]:[];if(additive&&store.setSketchMultiSelectEnabled)store.setSketchMultiSelectEnabled(true,false);legacyObjectSelect(ref.ownerId,false,false);const next={sketchId:ref.ownerId,kind:ref.targetKind.toLowerCase(),elementId:ref.targetId};if(additive){store.selection.sketchElements=preserved;const index=store.selection.sketchElements.findIndex(item=>item.kind===next.kind&&item.elementId===next.elementId);if(index>=0)store.selection.sketchElements.splice(index,1);else store.selection.sketchElements.push(next);}else store.selection.sketchElements=[next];store.selection.sketchElement=store.selection.sketchElements.at(-1)??null;syncSelectionRefs();if(notify)store.emit('selectionChanged',{sketchElement:store.selection.sketchElement?structuredClone(store.selection.sketchElement):null});return true;}if(!['OBJECT','SKETCH'].includes(ref.targetKind))return false;const object=store.getObject(ref.targetId);if(!object)return false;if(ref.targetKind==='SKETCH'&&object.type!=='sketch')return false;if(ref.targetKind==='OBJECT'&&object.type==='sketch')return false;legacyObjectSelect(ref.targetId,notify,additive);syncSelectionRefs();return true;};
...
const profilePathIdentityRegistrationLifecycle=installProfilePathIdentityRegistrationLifecycle(store);
const profilePathSelection=installProfilePathSelectionProjection(store,runtime,appUI);
